...
export function earnedPerkPoints(state: GameState) {
  ...
  const operationClears = Object.keys(state.rpg.completed).filter(id => !id.startsWith("gig-")).length * 2;
  const bossKills = Object.keys(state.rpg.completed).filter(id => id.startsWith("gig-")).length * 2;
...
function meetsNamedUnlock(state: GameState, perk: PerkDefinition) {
  ...
  if (perk.id === "solo-operation-breacher") return Object.keys(state.rpg.completed).some(id => !id.startsWith("gig-"));
  if (perk.id === "solo-critical-routine") return Object.keys(state.rpg.completed).some(id => !id.startsWith("gig-"));

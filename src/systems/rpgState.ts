...
export function createRpgState(): RpgState {
  return { level: 1, xp: 0, attributes: { body: 3, reflexes: 3, intelligence: 3, technical: 3, cool: 3 },
...
export function cloneRpgState(state?: RpgState): RpgState {
  ...
  return { ...base, ...state, attributes: { ...base.attributes, ...state?.attributes }, perks: { ...state?.perks },
...
export function normalizeRpgState(state?: RpgState): RpgState {
  ...
  if (next.active) {
    ...
    if (!mission || !["briefing", "combat", "decision", "failed"].includes(e.phase)) next.active = null;
      ...
      e.turn = integer(e.turn, 0, 100000); e.ram = integer(e.ram, 0, 17); e.meds = integer(e.meds, 0, 4);

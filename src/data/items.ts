...
const EQUIPMENT_MAX_UPGRADE_LEVEL = 5;
const upgradeableEquipmentTypes = new Set<ItemDefinition["type"]>(["Weapon", "Armor", "Cyberware"]);
...
export const items: ItemDefinition[] = [
  ...
  districtMaterial("smuggler-seal", "Smuggler Seal", "A stamped seal used to certify underpass cargo chains.", "Underpass Market fixer work and combat encounters.", 155),
  ...
  districtMaterial("daemon-fragment", "Daemon Fragment", "A dangerous fragment of hostile Blacknet logic.", "Blacknet Quarter combat encounters and bosses.", 240, "Epic"),
  districtMaterial("executive-processor", "Executive Processor", "A corporate processor used in clean-room tech.", "Glassline District elite guards and combat encounters.", 230, "Epic"),
  ...
  districtMaterial("security-override-chip", "Security Override Chip", "A restricted chip for bypassing corporate locks.", "Glassline District hacks and combat routes.", 220, "Epic"),
  ...
  districtMaterial("neural-dampener", "Neural Dampener", "A dampener used to control Neural Instability spikes.", "Helix Ward treatment chains and combat encounters.", 185),
  ...
  districtMaterial("ballistic-core", "Ballistic Core", "A hardened core used in late combat weapon chains.", "Redline Blocks enemies and combat encounters.", 205, "Epic"),
  ...
  districtMaterial("armor-breaker-plate", "Armor Breaker Plate", "A shaped plate used in armor-breaking ammunition and weapons.", "Redline Blocks combat encounters and bosses.", 215, "Epic"),
  ...
  districtMaterial("legendary-chrome-matrix", "Legendary Chrome Matrix", "A legendary matrix for endgame cyberware assembly.", "Skyline Core combat encounters and relic chains.", 640, "Legendary"),
  ...
  accessComponent("market-pass", "Market Pass", "Permanent contraband-route credential for Underpass Market combat encounters.", 100),
  component("glassline-access-token", "Glassline Access Token", "Corporate access token for extraction combat encounters.", 180),
  ...
  weapon("blacknet-spike-deck", "Blacknet Spike Deck", "A hostile deck weapon built for relay fights.", 3, "cyberdeckWeapons", { damage: 12, accuracy: 4, heatModifier: -0.02, neuralInstabilityModifier: 2 }, ["blacknet", "illegal", "ranged"], ["link", "batteryCore"], 2, "Blacknet combat reward."),
  ...
  weapon("heavy-breacher", "Heavy Breacher", "A heavy weapon for bosses, doors, and terrible decisions.", 3, "heavyWeapons", { damage: 24, attackSpeed: 420, armorPenetration: 6, heatModifier: 0.12 }, ["ranged", "loud", "highHeat", "boss"], ["muzzle", "barrel", "stock", "batteryCore"], 2, "combat drop."),
  ...expandedWeapons(),
  ...
  attachment("blacknet-link", "Blacknet Link", "Routes weapon telemetry through hostile relays.", "link", "Rare", ["cyberdeckWeapons", "smartWeapons"], { skillXp: { hacking: 0.03 }, heatGain: -0.02 }, ["blacknet"], "Blacknet combat drop."),
  ...
  attachment("prototype-overclock-core", "Prototype Overclock Core", "Unstable power core with strong upside.", "batteryCore", "Prototype", ["exoticWeapons", "heavyWeapons", "cyberdeckWeapons"], { combatDamage: 0.06, neuralInstabilityGain: 0.03 }, ["prototype", "boss"], "Rare combat drop."),
  ...
  weaponMod("boss-breaker", "Boss Breaker", "Reinforced internals for combat bosses.", "Rare", ["heavyWeapons", "sniperRifles", "techWeapons"], { combatDamage: 0.05 }, "Boss and combat damage.", ["boss"]),
...
items.forEach((item) => {
  if (upgradeableEquipmentTypes.has(item.type)) item.maxUpgradeLevel = EQUIPMENT_MAX_UPGRADE_LEVEL;
});
...
function cyberware(id: string, name: string, description: string, slot: CyberwareSlot, tier: number, requiredLevel: number, modifiers: ItemDefinition["modifiers"], instabilityLoad: number, tags: string[]): ItemDefinition {
  return { id, name, description, type: "Cyberware", rarity: tier >= 2 ? "Rare" : "Uncommon", tags: ["cyberware", ...tags], stackable: false, sellValue: 120 * tier, sourceHint: "Crafted from cyberware recipes.", slot, tier, requiredSkill: "cyberware", requiredLevel, modifiers, instabilityLoad, maxUpgradeLevel: EQUIPMENT_MAX_UPGRADE_LEVEL };
}
...
function weaponMod(
  ...
  return {
    ...
    sourceHint: "Weapon mod drop, combat reward, or specialist vendor.",
    modifiers,
    compatibleWeaponClasses,
    specialEffect,
    maxUpgradeLevel: 5,
...
function cyberwareFromSpec(spec: CyberwareSpec): ItemDefinition {
  return {
    ...
    maxUpgradeLevel: EQUIPMENT_MAX_UPGRADE_LEVEL,
  };
}
...
function expandedArmorAndAccessories(): ItemDefinition[] {
  return [
    ...
    gear("glassline-tactical-mask", "Glassline Tactical Mask", "Clean-room mask with threat tags and trace shielding.", "head", "Epic", 4, 28, { armor: 5, accuracy: 6, critChance: 0.03 }, { heatGain: -0.03, jobSuccessChance: 0.02 }, ["head", "corporate", "stealth"], "Glassline combat reward or Helix/Glassline blueprint craft."),
    gear("apex-response-helmet", "Apex Response Helmet", "An elite response helmet built for bosses and combat encounters.", "head", "Legendary", 5, 48, { armor: 8, accuracy: 8, critChance: 0.04 }, { damageReduction: 0.03, dropChance: 0.02 }, ["head", "boss", "combat"], "Skyline Core boss and legendary blueprint source."),
    ...
    gear("chromeweave-armor", "Chromeweave Armor", "Flexible chrome fabric that resists cuts and heat.", "chest", "Epic", 4, 30, { armor: 11, maxHp: 42, dodge: 0.02 }, { heatGain: -0.03 }, ["chest", "heat", "combat"], "Underpass and Glassline combat source."),
    gear("blacksite-assault-shell", "Blacksite Assault Shell", "Heavy assault shell for sustained combat fights.", "chest", "Legendary", 5, 52, { armor: 17, maxHp: 72, armorPenetration: 2 }, { damageReduction: 0.04 }, ["chest", "boss", "armor"], "Skyline and Redline boss reward."),
    gear("adaptive-combat-plating", "Adaptive Combat Plating", "Prototype plates that stiffen under incoming fire.", "chest", "Prototype", 6, 68, { armor: 22, maxHp: 96, dodge: -0.02 }, { combatDefense: 0.08, actionSpeed: -0.01 }, ["chest", "prototype", "risk"], "Prototype combat craft using legendary chrome matrix."),
    ...
    gear("reflex-grip-gauntlets", "Reflex Grip Gauntlets", "Gauntlets tuned for fast reloads and blade draws.", "hands", "Epic", 4, 32, { damage: 5, accuracy: 5, critChance: 0.04 }, { actionSpeed: 0.025 }, ["hands", "reflex"], "Redline combat encounters and rare weapon crafting."),
    ...
    gear("ghoststep-leg-rig", "Ghoststep Leg Rig", "Quiet leg rig for stealth and low-Heat work.", "legs", "Epic", 4, 34, { dodge: 0.07, armor: 4 }, { heatGain: -0.04, jobSuccessChance: 0.03 }, ["legs", "stealth"], "Underpass private buyer and combat source."),
    ...
    gear("shock-runner-boots", "Shock Runner Boots", "Insulated boots for drone yards and bad weather.", "boots", "Epic", 4, 34, { armor: 4, dodge: 0.07 }, { combatDefense: 0.02, actionSpeed: 0.02 }, ["boots", "drone"], "Rust Yards combat and drone source."),
    ...
    gear("sim-cache-catalyst", "Sim Cache Catalyst", "A small processor that improves simulated work.", "accessory1", "Epic", 4, 36, {}, { simCacheEfficiency: 0.06, masteryXpGain: 0.03 }, ["accessory", "simulation"], "Blacknet Quarter combat reward."),
...
function expandedWeapons(): ItemDefinition[] {
  return [
    ...
    customWeapon("ghost-market-vector", "Ghost Market Vector", "A compact illegal SMG with strong handling.", "Epic", 4, 34, "smgs", { damage: 18, attackSpeed: -220, accuracy: 5, dodge: 0.03, heatModifier: -0.03 }, ["ranged", "blackmarket", "stealth"], ["muzzle", "scope", "magazine", "grip", "stock"], 3, "Underpass combat and private buyer source."),
    ...
    customWeapon("blacksite-lmg", "Blacksite LMG", "A legendary heavy weapon for combat bosses.", "Legendary", 5, 62, "heavyWeapons", { damage: 46, attackSpeed: 620, armorPenetration: 14, accuracy: 2, dodge: -0.04, heatModifier: 0.16 }, ["ranged", "heavy", "boss"], ["muzzle", "scope", "barrel", "stock", "batteryCore"], 3, "Redline/Skyline combat boss source."),

    customWeapon("glassline-sniper", "Glassline Sniper", "A clean rifle made for corporate weakpoints.", "Epic", 4, 38, "sniperRifles", { damage: 34, attackSpeed: 460, accuracy: 12, critChance: 0.18, critDamage: 0.55 }, ["ranged", "corporate", "boss"], ["muzzle", "scope", "barrel", "stock"], 3, "Glassline guard captain and combat drop."),
    ...
    customWeapon("daemon-exec-deck", "Daemon Exec Deck", "A relic-grade weaponized deck for hostile Blacknet entities.", "Relic", 7, 92, "cyberdeckWeapons", { damage: 44, accuracy: 10, critChance: 0.12, neuralInstabilityModifier: 6, heatModifier: -0.06 }, ["blacknet", "relic", "boss"], ["link", "batteryCore"], 3, "Blacknet/Skyline relic combat source."),
    ...
    customWeapon("black-badge-carbine", "Black Badge Carbine", "A corporate security rifle with doctrine-coded recoil.", "Legendary", 5, 60, "assaultRifles", { damage: 38, attackSpeed: 10, accuracy: 15, armor: 4, critChance: 0.08 }, ["ranged", "assault", "corporate", "combat"], ["muzzle", "scope", "magazine", "grip", "barrel", "stock"], 3, "Glassline combat boss drop and corporate fixer high-rank contracts.", "Security Doctrine: improves corporate and combat success.", { jobSuccessChance: 0.04, damageReduction: 0.03 }),
    customWeapon("overwatch-helix", "Overwatch Helix", "A biometric adaptive rifle tuned by clinic telemetry.", "Prototype", 6, 82, "assaultRifles", { damage: 44, attackSpeed: 30, accuracy: 13, critChance: 0.1, maxHp: 24, neuralInstabilityModifier: 2 }, ["ranged", "assault", "medical", "prototype", "boss"], ["scope", "magazine", "grip", "barrel", "stock", "link"], 3, "Helix Ward prototype combat and advanced prototype crafting.", "Biometric Feedback: kills restore HP; boss hits improve with installed cyberware.", { combatDamage: 0.07, healingReceived: 0.04 }),
    ...
    customWeapon("redline-siege-platform", "Redline Siege Platform", "A gang-grade heavy weapon built to hold a block by itself.", "Legendary", 5, 64, "heavyWeapons", { damage: 54, attackSpeed: 650, accuracy: 2, armorPenetration: 15, maxHp: 42, heatModifier: 0.16 }, ["ranged", "lmg", "heavy", "redline", "combat"], ["muzzle", "scope", "barrel", "stock", "batteryCore"], 3, "Redline Blocks combat and Redline Saints high-rank rewards.", "Siege Rhythm: consecutive kills slowly increase damage until combat stops.", { combatDamage: 0.08 }),
    ...
    customWeapon("midnight-protocol", "Midnight Protocol", "A black-ops sniper rifle designed for silent combat routes.", "Legendary", 5, 66, "sniperRifles", { damage: 58, attackSpeed: 540, accuracy: 19, critChance: 0.18, critDamage: 0.8, armorPenetration: 13, heatModifier: -0.08 }, ["ranged", "sniper", "stealth", "assassination", "combat"], ["muzzle", "scope", "barrel", "stock"], 3, "High-threat corporate combat and legendary sniper blueprint.", "Silent Protocol: stealth and assassination combat routes improve with a suppressor equipped.", { jobSuccessChance: 0.04, combatDamage: 0.05 }),
    ...
    customWeapon("jackal-maul", "Jackal Maul", "The Chrome Jackals signature hammer, all weight and warning paint.", "Legendary", 5, 60, "bluntWeapons", { damage: 50, attackSpeed: 380, accuracy: 3, armorPenetration: 22, maxHp: 48 }, ["melee", "blunt", "rust", "armored", "mech"], ["grip", "batteryCore"], 3, "Chrome Jackals rank reward and Rust Yards combat boss drop.", "Jackal Crush: bonus damage against armored and mechanical enemies.", { skillXp: { scavenging: 0.04 }, combatDamage: 0.06 }),
    ...
    customWeapon("white-arc-lance", "White Arc Lance", "A clean corporate-grade tech weapon with surgical charge control.", "Legendary", 5, 68, "techWeapons", { damage: 56, attackSpeed: 340, accuracy: 11, armorPenetration: 28, critDamage: 0.35 }, ["ranged", "tech", "charged", "corporate", "combat"], ["scope", "barrel", "stock", "batteryCore"], 3, "Glassline boss drop and Corporate Supplier special stock after requirements.", "Arc Lance: combat bosses take increased damage from charged shots.", { combatDamage: 0.08 }),
    customWeapon("nullbreaker-coilgun", "Nullbreaker Coilgun", "An unstable prototype rail weapon that punches through rules and armor.", "Prototype", 6, 92, "techWeapons", { damage: 76, attackSpeed: 510, accuracy: 10, armorPenetration: 38, critChance: 0.1, neuralInstabilityModifier: 5 }, ["ranged", "tech", "prototype", "armorPiercing", "boss"], ["scope", "barrel", "stock", "batteryCore"], 3, "Blacksite prototype crafting and rare Glassline high-threat combat drop.", "Nullbreak Overcharge: massive armor-piercing shot chance, adding Instability.", { combatDamage: 0.1, neuralInstabilityGain: 0.03 }),
...
function expandedCyberware(): ItemDefinition[] {
  return [
    ...
    chrome("reflex-prediction-core", "Reflex Prediction Core", "Predicts incoming hits before muscles commit.", "neural", "Epic", 4, 42, { dodgeChance: 0.04, combatDamage: 0.06 }, 9, ["neural", "reflex", "combat"], "Redline combat and prototype implant craft."),
    ...
    chrome("overclocked-neural-lattice", "Overclocked Neural Lattice", "Prototype neural lattice with dangerous upside.", "neural", "Prototype", 6, 78, { actionSpeed: 0.09, combatDamage: 0.1, neuralInstabilityGain: 0.03 }, 18, ["neural", "prototype", "risk"], "Prototype combat chain."),
    ...
    chrome("smart-targeting-array", "Smart Targeting Array", "Improves smart weapon tracking and crit setup.", "optics", "Epic", 4, 40, { combatDamage: 0.06, dropChance: 0.03 }, 9, ["optics", "smart"], "Blacknet combat drop."),
    ...
    chrome("overload-arm-frame", "Overload Arm Frame", "Prototype arm frame with dangerous strength spikes.", "arms", "Prototype", 6, 74, { combatDamage: 0.12, skillRewards: 0.04, neuralInstabilityGain: 0.03 }, 18, ["arms", "prototype"], "Redline prototype combat."),
    ...
    chrome("adaptive-skin-plating", "Adaptive Skin Plating", "Epic skin plating that shifts against incoming impacts.", "skin", "Epic", 4, 44, { combatDefense: 0.08, damageReduction: 0.03 }, 10, ["skin", "plating"], "Helix/Glassline combat source."),
    chrome("reactive-chrome-skin", "Reactive Chrome Skin", "Prototype chrome skin that blooms under fire.", "skin", "Prototype", 6, 80, { combatDefense: 0.12, damageReduction: 0.05, neuralInstabilityGain: 0.02 }, 18, ["skin", "prototype"], "Prototype medical combat source."),
    ...
    chrome("rogue-protocol-kernel", "Rogue Protocol Kernel", "Prototype kernel that trades stability for power.", "operatingSystem", "Prototype", 6, 82, { skillXp: { hacking: 0.1 }, actionSpeed: 0.06, neuralInstabilityGain: 0.04 }, 20, ["os", "prototype", "risk"], "Blacknet relic combat."),
    ...
    chrome("sim-cache-optimizer", "Sim Cache Optimizer", "Optimizes simulated work and mastery replay.", "utility", "Legendary", 5, 70, { simCacheEfficiency: 0.1, masteryXpGain: 0.06 }, 13, ["utility", "simulation"], "Skyline luxury and Blacknet combat source."),
...
function chrome(id: string, name: string, description: string, slot: CyberwareSlot, rarity: ItemRarity, tier: number, requiredLevel: number, modifiers: ItemDefinition["modifiers"], instabilityLoad: number, tags: string[], sourceHint: string): ItemDefinition {
  return { id, name, description, type: "Cyberware", rarity, tags: ["cyberware", ...tags], stackable: false, sellValue: raritySellValue(rarity, 125, tier), sourceHint, slot, tier, requiredSkill: "cyberware", requiredLevel, modifiers, instabilityLoad, maxUpgradeLevel: EQUIPMENT_MAX_UPGRADE_LEVEL };
}

...
export const progressionTiers = [
  ...
  { id: "tier1", name: "Tier 1: Street Operator", goals: ["Any skill level 10", "100 total actions", "25 enemy kills", "First cyberware equipped", "First mission completed"] },
  ...
  { id: "tier3", name: "Tier 3: Chrome Specialist", goals: ["Any skill level 50", "Several action masteries above level 25", "First main mission completed", "First rare blueprint found", "First preset saved", "First mastery pool checkpoint at 50%"] },
...
export function tierProgress(state: GameState, tierId: string) {
  ...
  const checks: Record<string, boolean[]> = {
    ...
    tier1: [anySkill >= 10, actions >= 3, kills >= 25, Object.keys(state.equippedCyberware).length > 0, Object.keys(state.rpg.completed).length > 0],
    ...
    tier3: [anySkill >= 50, Object.values(state.actionMastery).filter((m) => m.level >= 25).length >= 3, Object.keys(state.rpg.completed).some(id => !id.startsWith("gig-")), Object.keys(state.unlockedBlueprints).length > 0, Object.keys(state.equipmentPresets).length >= 5, pool50],

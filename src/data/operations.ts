...
export const operations: OperationDefinition[] = [
  ...
  },
  {
    ...
    routes: [
      ...
      { id: "blacknetBreach", name: "Blacknet Breach", description: "Hijack drone routing before the warden boots.", requirements: ["Hacking level 15 recommended"], recommendedTags: ["blacknet", "cyberdeckWeapons", "shock"], successModifier: 0.05, neuralInstabilityChange: 1, rareDropModifier: 0.03 },
  ...
  },
  {
    ...
    routes: [
      { id: "blacknetBreach", name: "Blacknet Breach", description: "Run the relay as a hacking route for data and unique drops.", requirements: ["Hacking level 25"], recommendedTags: ["cyberdeckWeapons", "blacknet", "trace-scrambler"], successModifier: 0.08, neuralInstabilityChange: 2, rewardMultiplier: 1.12, rareDropModifier: 0.05 },
  ...
  },
  {
    ...
    routes: [
      ...
      { id: "blacknetBreach", name: "Blacknet Breach", description: "Open facility routes through security daemons.", requirements: ["Hacking level 35"], recommendedTags: ["cyberdeckWeapons", "blacknet", "tech"], successModifier: 0.07, neuralInstabilityChange: 2, rareDropModifier: 0.04 },
    ],
...
function makeExpandedOperations(): OperationDefinition[] {
  ...
  op("op-redline-initiation", "Redline Initiation", "neonRow", 32, "Disrupt a Redline recruitment night before the block turns hostile.", "boss-redline-initiate", ["boosted-thug", "neon-lookout"], { credits: 260, reputation: 15, cyberwareParts: 6 }, "bounty-token"),
  op("op-neon-row-lockdown", "Neon Row Lockdown", "neonRow", 50, "Push through a district-wide sweep and pull the crew command node offline.", "boss-neon-lockdown-sergeant", ["clinic-shaker", "neon-lookout", "scrap-drone"], { credits: 420, reputation: 22, encryptedData: 8 }, "faction-authorization"),
  ...
  op("op-chrome-jackals-trial", "Chrome Jackals Trial", "rustYards", 45, "Survive a Jackal trial run to earn access to better vehicle work.", "boss-jackal-trialmaster", ["yard-raider", "scrap-hound", "jackal-roadboss"], { vehicleParts: 34, reputation: 18, armorPlating: 4 }, "smugglerCompartment"),
  op("op-warden-scrap-fortress", "Warden's Scrap Fortress", "rustYards", 65, "Crack the fortress yard where every wall is a salvaged weapon.", "boss-scrap-fortress-core", ["mag-clamp-loader", "jackal-roadboss", "scrap-hound"], { vehicleParts: 50, credits: 520, reputation: 24 }, "prototypeDriveUnit"),

  op("op-ledger-knife-run", "Ledger Knife Run", "underpassMarket", 18, "Steal a broker ledger before the sellers erase each other.", "boss-ghost-ledger-keeper", ["market-cutthroat", "ledger-thief"], { credits: 240, encryptedData: 8, reputation: 12 }, "private-buyer-contact"),
  op("op-tunnel-auction-crash", "Tunnel Auction Crash", "underpassMarket", 30, "Crash a sealed auction without letting the market identify your buyer.", "boss-tunnel-auctioneer", ["ledger-thief", "tunnel-gunner", "market-cutthroat"], { credits: 360, reputation: 16, encryptedData: 8 }, "rare-listing-permit"),
  op("op-contraband-surgery", "Contraband Surgery", "underpassMarket", 42, "Raid an illegal operating theater trading in prototype cores.", "boss-contraband-surgeon", ["contraband-sawbones", "tunnel-gunner"], { credits: 440, cyberwareParts: 8 }, "prototype-neural-core"),
  op("op-underpass-kingpin", "Underpass Kingpin", "underpassMarket", 62, "Break a private market protection ring before it taxes every route.", "boss-underpass-kingpin", ["ghost-market-factor", "contraband-sawbones", "tunnel-gunner"], { credits: 720, reputation: 28, encryptedData: 16 }, "boss-data-key"),

  op("op-firewall-breach", "Firewall Breach", "blacknetQuarter", 30, "Cut through a living firewall and harvest its exposed routines.", "boss-firewall-warden", ["trace-avatar", "packet-wraith"], { encryptedData: 35, credits: 260, reputation: 14 }, "trace-scrambler"),
  op("op-null-choir-initiation", "Null Choir Initiation", "blacknetQuarter", 40, "Survive a Null Choir initiation route without losing your signal identity.", "boss-null-choir-precentor", ["choir-proxy", "packet-wraith"], { encryptedData: 45, reputation: 18 }, "daemon-chain-program"),
  op("op-rogue-daemon-hunt", "Rogue Daemon Hunt", "blacknetQuarter", 55, "Hunt a daemon eating abandoned relay bodies across the Quarter.", "boss-rogue-daemon", ["daemon-butcher", "choir-proxy", "trace-avatar"], { encryptedData: 60, credits: 420, reputation: 22 }, "prototype-weapon-core"),
  op("op-deep-static-collapse", "Deep Static Collapse", "blacknetQuarter", 75, "Collapse a deep static pocket before it rewrites local access.", "boss-deep-static-heart", ["null-oracle-shell", "daemon-butcher", "packet-wraith"], { encryptedData: 90, reputation: 32, credits: 650 }, "boss-data-key"),

  op("op-glassline-audit-night", "Glassline Audit Night", "glasslineDistrict", 35, "Slip through an after-hours audit where every locked door has a gun behind it.", "boss-audit-knight", ["corp-response-guard", "glassline-auditor"], { credits: 620, encryptedData: 12, reputation: 16 }, "corporate-access-token"),
  op("op-sterile-wing-breach", "Sterile Wing Breach", "glasslineDistrict", 45, "Breach a sterile wing and steal implant frames before the drones purge inventory.", "boss-sterile-captain", ["sterile-drone", "corp-response-guard"], { credits: 780, cyberwareParts: 14, reputation: 18 }, "stabilized-chrome-frame"),
  op("op-executive-proxy-war", "Executive Proxy War", "glasslineDistrict", 58, "Fight through proxy security for a boardroom no one admits exists.", "boss-executive-proxy", ["executive-bodyguard", "glassline-auditor", "sterile-drone"], { credits: 980, reputation: 24, encryptedData: 18 }, "glassline-access-token"),
  op("op-prototype-directive", "Prototype Directive", "glasslineDistrict", 80, "Extract prototype directives from a facility designed to burn itself clean.", "boss-prototype-director", ["prototype-handler", "executive-bodyguard", "sterile-drone"], { credits: 1400, reputation: 36, prototypeDriveUnit: 1 }, "bp-corporate-cyberware"),

  op("op-helix-triage-case", "Helix Triage Case", "helixWard", 16, "Stabilize a clinic breach before desperate patients become armed witnesses.", "boss-helix-triage-chief", ["clinic-shaker", "contraband-sawbones"], { credits: 160, cyberwareParts: 3, reputation: 10 }, "medical-access-pass"),
  op("op-stabilizer-vault", "Stabilizer Vault", "helixWard", 28, "Recover stolen stabilizers from a locked recovery vault.", "boss-stabilizer-matron", ["sterile-drone", "contraband-sawbones"], { credits: 260, cyberwareParts: 6 }, "advanced-stabilizer"),
  op("op-recovery-proxy", "Recovery Proxy", "helixWard", 42, "Disable a recovery proxy that is billing patients into debt.", "boss-recovery-proxy", ["sterile-drone", "glassline-auditor"], { credits: 420, reputation: 18, cyberwareParts: 10 }, "stabilized-chrome-frame"),
  op("op-neural-quietus", "Neural Quietus", "helixWard", 60, "Shut down a quietus protocol before it wipes unstable runners.", "boss-neural-quietus", ["prototype-handler", "sterile-drone", "choir-proxy"], { credits: 700, reputation: 26, encryptedData: 18 }, "neural-dampener-blueprint"),
  op("op-helix-clean-room-siege", "Helix Clean-Room Siege", "helixWard", 78, "Defend a clean-room against mercs trying to steal living prototypes.", "boss-prototype-director", ["prototype-handler", "executive-bodyguard", "contraband-sawbones"], { credits: 1100, reputation: 34, cyberwareParts: 18 }, "bp-stabilized-buffer"),

  op("op-redline-bounty-board", "Redline Bounty Board", "redlineBlocks", 24, "Clear a stack of open bounties before the board changes hands.", "boss-bounty-captain", ["bounty-scout", "redline-brawler"], { credits: 260, reputation: 16 }, "bounty-token"),
  op("op-saints-ring", "Saints Ring", "redlineBlocks", 36, "Win a closed-ring crew challenge under live-fire rules.", "boss-saints-ringmaster", ["saints-shotcaller", "redline-brawler"], { credits: 380, reputation: 20, cyberwareParts: 6 }, "servo-knuckles"),
  op("op-blood-contract", "Blood Contract", "redlineBlocks", 52, "Take a contract that pays only if the whole block hears about it.", "boss-blood-contract", ["bloodsport-chrome", "saints-shotcaller"], { credits: 620, reputation: 28 }, "faction-authorization"),
  op("op-redline-execution", "Redline Execution", "redlineBlocks", 72, "Stop an executioner before their bounty ledger becomes district law.", "boss-redline-executioner", ["redline-executioner", "bloodsport-chrome", "bounty-scout"], { credits: 980, reputation: 38, cyberwareParts: 14 }, "boss-data-key"),
  op("op-redline-block-war", "Redline Block War", "redlineBlocks", 90, "Push through a full crew war and claim the board for yourself.", "boss-redline-executioner", ["redline-executioner", "saints-shotcaller", "bloodsport-chrome"], { credits: 1500, reputation: 50 }, "rare-blueprint-fragment"),

  op("op-skyline-concierge", "Skyline Concierge", "skylineCore", 50, "Survive a luxury concierge screening where rejection is lethal.", "boss-skyline-concierge", ["executive-bodyguard", "glassline-auditor"], { credits: 1000, reputation: 22 }, "district-permit"),
  op("op-luxury-assessment", "Luxury Assessment", "skylineCore", 65, "Pass an elite assessment by stealing the assessor's own risk ledger.", "boss-luxury-assessor", ["prototype-handler", "executive-bodyguard"], { credits: 1400, reputation: 28 }, "rare-blueprint-fragment"),
  op("op-private-armory", "Private Armory", "skylineCore", 82, "Breach a private armory hidden behind membership contracts.", "boss-private-armory", ["redline-executioner", "prototype-handler", "executive-bodyguard"], { credits: 1900, reputation: 36, prototypeDriveUnit: 1 }, "prototypeDriveUnit"),
  op("op-skyline-blackout", "Skyline Blackout", "skylineCore", 100, "Cut power to the top of the city and raid the panic rooms in the dark.", "boss-skyline-blackout", ["null-oracle-shell", "prototype-handler", "redline-executioner"], { credits: 2800, reputation: 55 }, "boss-data-key"),
  op("op-executive-afterimage", "Executive Afterimage", "skylineCore", 115, "Hunt the afterimage of an executive route through Blacknet, luxury, and gunfire.", "boss-skyline-blackout", ["null-oracle-shell", "ghost-market-factor", "prototype-handler"], { credits: 3600, reputation: 70, prototypeDriveUnit: 1 }, "rare-blueprint-fragment"),
  ];
}
